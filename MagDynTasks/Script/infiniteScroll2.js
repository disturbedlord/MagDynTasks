...
async function loadNextPage(direction) {
  ...
  switch (direction) {
    ...
    case "up": {
      ...
      const data = pageCache.get(pageToPrepend);
      if (!data) return;
      ...
      Array.from(measureContainer.children).forEach((tr) => {
        realAddedHeight += tr.offsetHeight;
      });
      ...
      // ── 🔥 CRITICAL: Force Clusterize to re-render ──────────────
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          // Step 1: restore scroll
          scrollEl.scrollTop = prevScrollTop + realAddedHeight;

          // Step 2: tiny nudge to trigger Clusterize recalculation
